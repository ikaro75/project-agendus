 ...
 */
package mx.edu.ilce.intranet.comunidad;

import java.sql.ResultSet;
import java.text.SimpleDateFormat;
import mx.edu.ilce.intranet.agenda.Actividad;
import mx.edu.ilce.intranet.agenda.CategoriaActividad;
...
public class ActividadContacto extends Consulta {
    ...
    public String insert() throws Fallo {
        ...
        Consulta cnsNota=new Consulta(273, "insert", "0", "", super.getUsuario());
        cnsNota.setRegistros();
        
        try {

            //1. Inserta nueva actividad del contacto con quien se comparte
            frmActividad = new Forma(new Consulta(101,"insert", "0", "", super.getUsuario()), false);
            ...
            if (this.claveContacto != null) {
                ...
                if (!rs.next()) {
                    frmCategoriaActividad = new Forma(new Consulta(309, "insert", "0", "", super.getUsuario()), false);
                ...
                if (c.getEmail()!= null) {
                    mail.sendEmail("intranet@ilce.edu.mx", c.getEmail(),
                            super.getUsuario().getNombre().concat(" ").concat(super.getUsuario().getApellidos()).concat(" te ha agendado la actividad ").concat(a.getActividad()),
                            notificacion.concat(super.getUsuario().getNombre()), null);
                } else {
                    q.append("<warning><![CDATA[El usuario ").append(c.getNombre()).append(" ").append(c.getApellidos()).append(" no tiene un correo registrado y no fue posible notificarsele.").append("]]></warning>");
                }
                ...
                while (rs.next()) {
                    ...
                    cnsNota.getCampos().get("titulo").setValor(remitente.getNombre().concat(" ").concat(remitente.getApellidos()));
