...
/**
    ...
    public String insert(Usuario destinatario, String entidad, Boolean propaga) throws Fallo {
        ...
        try {
            ...
            notificacion = u.getNombre().concat(" ").concat(u.getApellidos()).concat(" ha publicado un comentario ").concat((!entidad.equals("")?" en ".concat(entidad):"" )).concat(":<br /><br /><strong>").concat(this.mensaje).concat("</strong>");
                ...
                while (rs.next()) {
                     ...
                     if (c.getEmail()!=null) {
                        mail.sendEmail("intranet@ilce.edu.mx", c.getEmail(),
                            u.getNombre().concat(" ").concat(u.getApellidos()).concat(" ha publicado un comentario").concat((!entidad.equals("")?" en ".concat(entidad):"" )),
                            notificacion, null);
                        resultadoXML.append("<notificacion><![CDATA[Se envió la notificacion '").append(notificacion).append("]]></notificacion>");
                     } else {
                        resultadoXML.append("<warning><![CDATA[El usuario ").append(c.getNombre()).append(" ").append(c.getApellidos()).append(" no tiene un correo registrado y no fue posible notificarsele.").append("]]></warning>");
                     }
                }
                ...
                if (destinatario.getClave()!=this.claveEmpleado) {
                    if (destinatario.getEmail()!=null) {
                        mail.sendEmail("intranet@ilce.edu.mx", destinatario.getEmail(),
                            super.getUsuario().getNombre().concat(" ").concat(super.getUsuario().getApellidos()).concat(" ha publicado un comentario").concat((!entidad.equals("")?" en ".concat(entidad):"" )),
                            notificacion, null);                            
                        resultadoXML.append("<notificacion><![CDATA[Se envió la notificacion '").append(notificacion).append("' a ").append(destinatario.getNombre()).append(" ").append(destinatario.getApellidos()).append("]]></notificacion>");
                    }  else {
                        resultadoXML.append("<warning><![CDATA[El usuario ").append(destinatario.getNombre()).append(" ").append(destinatario.getApellidos()).append(" no tiene un correo registrado y no fue posible notificarsele.").append("]]></warning>");
                    }
                }                 
            }
        } catch (Exception e) {
