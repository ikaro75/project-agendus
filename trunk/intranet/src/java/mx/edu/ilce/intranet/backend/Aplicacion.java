...
/**
    ...
    public Aplicacion(Integer claveAplicacion, Usuario usuario) {
        ...
        try {
           ResultSet rs = super.getUsuario().getCx().getRs("select *,(select forma from be_forma where clave_forma=be_aplicacion.clave_forma_principal) as  forma_principal from be_aplicacion where clave_aplicacion=".concat(claveAplicacion.toString()));
     ...
     * @param c     Instancia de clase <code>mx.edu.ilce.intranet.modelo.Consulta</code>
     * @param cx    instancia de clase <code>mx.edu.ilce.intranet.modelo.Conexion</code>
     */
     ...
     * @throws Fallo        Objeto <code>mx.edu.ilce.intranet.modelo.Fallo</code> Si ocurre un error al hacer operaciones en la base de datos
     */
