...
%><%@page import="java.io.File" %><%
    ...
    if (user.getFoto() != null) {
        File archivoOrigen = new File("c:\\intranet2.0\\".concat(user.getFoto().replaceAll("/intranet/temp/", "").replace("/","\\")));
		...
		debug = "<debug>Se encontró el archivo origen ".concat(archivoOrigen.getAbsolutePath()).concat("</debug>");
                sAppPath = application.getRealPath("/").replace("\\build\\web", "");
...
<qry source="(sesion de usuario)">
    <registro id="1">
        ...
        <apellidos tipo_dato="string"><![CDATA[ <%=user.getApellidos()%> ]]></apellidos>
        ...
        <sesion tipo_dato="bit"><![CDATA[<%=request.getSession().getId()%>]]></sesion>
        <%=debug%>
    </registro>
    <%=error%>
</qry>