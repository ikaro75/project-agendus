...
<%@page import="mx.edu.ilce.intranet.jasper.*"%>
...
<%   response.setContentType("application/pdf");
    ...
    try {
        ...
        File archivoReporte = new File("C:\\apache-tomcat-6.0\\webapps\\intranet\\reportes\\".concat(reporte.getJrxml()).concat(".jasper"));

        if (!archivoReporte.exists()) {
            debug.append("<debug>El archivo ").append("C:\\apache-tomcat-6.0\\webapps\\intranet\\reportes\\").append(reporte.getJrxml()).append(".jasper").append(" no se encuentra</debug>");
        }

        reporte.setPrint(user.getCx());
        JasperExportManager.exportReportToPdfFile(reporte.getPrint(), "C://apache-tomcat-6.0//webapps//intranet//temp//".concat(reporte.getJrxml()).concat(".pdf"));

        archivoReporte = new File("C:\\apache-tomcat-6.0\\webapps\\intranet\\temp\\".concat(reporte.getJrxml()).concat(".pdf"));
        if (!archivoReporte.exists()) {
            debug.append("<debug>El archivo ").append("C:\\apache-tomcat-6.0\\webapps\\intranet\\temp\\".concat(reporte.getJrxml()).concat(".pdf")).append(" no se encuentra</debug>");
        }

        InputStream entrada = new FileInputStream("C://apache-tomcat-6.0//webapps//intranet//temp//".concat(reporte.getJrxml()).concat(".pdf"));
