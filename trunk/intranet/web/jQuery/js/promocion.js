function prmcn_entregables_init() {
    $("#cantidad,#precio_unitario,#costo_directo,#tipo_cambio,#factor_riesgo,#factor_costo_operacion,#factor_recuperacion,#clave_impuesto").change(function() {
        ...
        var importe = costo_unitario * parseInt($("#cantidad").val()) * (1 + IVA);
        $("#importe").val(importe);
        ...
        $("#clave_producto").change(function() {
            var clave_producto=$(this).val();
            $.ajax(
            {
                url: "control?$cmd=plain&$cf=351&$pk=0&$ta=select&$w=clave_producto=" + clave_producto,
                ...
                success: function(data) {
                    if (typeof data == "string") {
                        ...
                        if (xmlProducto.parseError.errorCode > 0) {
                            alert("Error de compilación xml:" + xmlProducto.parseError.errorCode + "\nParse reason:" + xmlProductoInsumo.parseError.reason + "\nLinea:" + xmlProducto.parseError.line);
                        }
                    }
                    else {
                        xmlProducto = data;
                    ...
                    $("#costo_directo").val(formatCurrency($(xmlProducto).find("importe").text()));
                    $("#costo_directo" ).trigger( "change" );
                    /*xmlCategoriaInsumo.                
