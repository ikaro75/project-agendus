...
( function($) {
    $.fn.appgrid = function(opc){
        ...
        return this.each( function(){
            ...
            $(this).html("<table width='100%' id='grid"+ suffix +
                ...
                "' firstReportId='" + $.fn.appgrid.options.firstReportId  +
                "'>" +
    ...
    function processGridDefinition (data){
        ...
        $("#pager"+ suffix).attr("firstReportId", $(xml).find('reporte:first').attr("id"));
    ...
    $.fn.appgrid.handleGridDefinition = function(xml){
        var iCol=0;
        $("#_status_").val("Construyendo definición del catálogo");
        //Manejo de errores
        var error=$(xml).find("error");
    ...
    $.fn.appgrid.openKardex = function(nApp, nEntidad,sDateStamp, id) {
        ...
        if ($('#appTab_'+ nApp + "_" + nEntidad + "_"+ id).length) {
            $tabs.tabs( "select", '#appTab_'+ nApp + "_" + nEntidad + "_"+ id);
            return;
        }
        ...
        sTitulo=oGrid.jqGrid()[0].p.colNames[1] + ' ' + oGrid.getCell(nRow,1);
        //nEntidadKardex=$('#grid'+ suffix).jqGrid()[0].id.split("_")[2];
        
        $tabs.tabs( "add", "#appTab_"+ nApp + "_" + nEntidad + "_"+ id, sTitulo);
        $tabs.tabs( "select", "#appTab_"+ nApp + "_" + nEntidad + "_"+ id);
        oTabPanel=$("#appTab_"+ nApp + "_" + nEntidad + "_"+ id);
