        ...
        {   
            ...
            success:  function(data){
                ...
                $("#btnAnterior_"+ surveySuffix+", #btnSiguiente_"+ surveySuffix).button().click(function() {
                     ...
                     *3. Actualizar ex_respuesta_participante a través de la llamada a survey.jsp 
                     *   con el parametro $ta=save; survey.jsp debe proporcionar en su respuesta 
               ...
               $("#btnAutentifica_"+ surveySuffix).button().click(function() {
                    $.ajax(
                    {
                        url: "survey.jsp?$ccp="+ $.fn.survey.options.pk +"&$ta=valid-authentication&auth_code=" + $("#auth_code").val(), 
                        dataType: ($.browser.msie) ? "text" : "xml",
                        success:  function(data){
                              if (typeof data == "string") {
               ...
               $("#btn_ReenviaCodigoAutorizacion"+ surveySuffix).button().click(function() {
                    $.ajax(
                    {
                        ...
                        url: "survey.jsp?$ccp=" + surveySuffix + "&$ta=send-validation-code&" + $("#auth_code").val(), 
                        dataType: ($.browser.msie) ? "text" : "xml",
                        success:  function(data){
                              if (typeof data == "string") {
                                xmlmlx = new ActiveXObject("Microsoft.XMLDOM");
                ...
                function processXml(data) { 
                    ...
                    if (siguienteAccion=="continua") {
                        ...
                        $("#dlgModal_"+ surveySuffix).dialog("destroy");
                        $("#dlgModal_"+ surveySuffix).remove();
                        alert('Caso de uso finalizado exitosamente');
                    }          
